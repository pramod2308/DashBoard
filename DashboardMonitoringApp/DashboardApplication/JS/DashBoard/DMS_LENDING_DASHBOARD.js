var app = angular.module('app').controller('DMSLendingCtrl', ['$scope', '$http', 'ServiceCall', '$log', '$window', 'cache', function ($scope, $http, ServiceCall, $log, $window, cache) {
    ...
    $scope.changedValue = function (item) {
        ...
        item.TabName = 'LENDING_DASHBOARD';
        $scope.param = item;
        $('#loader').fadeIn();
        ...
        ServiceCall.PostData($scope.URL, $scope.param).then(function (data) {
            if (data != null || data != undefined) {
                ...
                $scope.Response = JSON.parse(data.DMSResponseData);
