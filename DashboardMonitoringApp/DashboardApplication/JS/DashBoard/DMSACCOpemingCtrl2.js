var app = angular.module('app').controller('DMSAccOpeningCtrl2', ['$scope', '$http', 'ServiceCall', '$log', '$window', 'cache', '$timeout', function ($scope, $http, ServiceCall, $log, $window, cache, $timeout) {
    ...
    $scope.changedValue2 = function () {
        ...
        var item = {};
        item.DMSDateTime = $scope.DMSDate;
        item.Tab = "AODashBoard_pendingByChannel";
        $scope.param = item;
        $('#loader').fadeIn();
        $scope.URL = "/DashboardAppService.svc/ProcessDMSAccountOpening";
        ServiceCall.PostData($scope.URL, $scope.param).then(function (data) {
